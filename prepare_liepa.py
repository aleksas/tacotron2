...
def get_data(voice, transcript_list):
    with open(os.path.join(voice, transcript_list), 'rt', encoding='utf-8') as fp:
        for idx, text in enumerate(fp.readlines()):
            yield os.path.join(voice, "%d.wav" % idx), text

def get_subsets(proportions, voice, transcript_list):
    data = list(get_data(voice, transcript_list))
    random.shuffle(data)
    start = 0
...
if __name__ == "__main__":
    ...
    subsets = get_subsets(proportions=proportions, voice=args.voice, transcript_list=args.transcript_list)
